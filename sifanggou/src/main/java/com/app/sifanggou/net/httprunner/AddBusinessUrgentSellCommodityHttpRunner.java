...
public class AddBusinessUrgentSellCommodityHttpRunner extends HttpRunner {
    @Override
    public void onEventRun(Event event) throws Exception {
        String commodityId = (String) event.getParamAtIndex(0);
        String sellPrice = (String) event.getParamAtIndex(1);
        String num = (String) event.getParamAtIndex(2);
        String validDeadline = (String) event.getParamAtIndex(3);

        HashMap<String, String> map = new HashMap<String, String>();
        map.put("commodityId", commodityId);
        map.put("sellPrice", sellPrice);
        map.put("num", num);
        map.put("validDeadline", validDeadline);

        String result = HttpUtils.doPost(URLUtils.ADDBUSINESSURGENTSELLCOMMODITY, map);
